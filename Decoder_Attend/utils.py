...
def draw_language_attention_map(attention, ques, path, step, out):
    # attention [1 ques_len]
    ques = ques.split(' ')
    plt.figure(figsize=(10, 7.5))

    ax = sns.heatmap(attention, cmap=plt.get_cmap('Blues'), linewidths=1, annot=True, fmt=".3f",
                     cbar_kws=dict(use_gridspec=False, location="bottom"), square=True)
    ax.set(yticklabels=[])  # 不显示y轴的刻度标签
    ax.tick_params(left=False, bottom=False)  # 不显示左下刻度线
    ax.set_xticks(([i+0.5 for i in range(len(ques))]))
    ax.set_xticklabels(ques)
    ax.set_title(f'output label:{out}')
    plt.savefig(os.path.join(path, f'{step}.jpg'))
    # plt.show()
    plt.close()
...
# , path, step, out
# out = '\sqrt'
# ques = '( - 1 \\frac { } 4 + 2 5 0 . ) \\times'
# attention = torch.randn(1, 14)
# draw_language_attention_map(attention, ques, out)
