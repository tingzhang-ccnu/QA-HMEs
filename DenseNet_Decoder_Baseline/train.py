...
def train(args, model, optimizer, epoch, train_loader, writer=None):
    ...
    with tqdm(train_loader, total=len(train_loader)) as pbar:
        for step, data in enumerate(pbar):
            ...
            img, img_mask, label, label_mask = data
            ...
            probs, loss = model(img, img_mask, label, label_mask)
            loss.backward()
...
def eval(args, model, epoch, test_loader, writer=None):
    ...
    with tqdm(test_loader, total=len(test_loader)) as pbar, torch.no_grad():
        for step, data in enumerate(pbar):
            ...
            probs, loss = model(img, img_mask, label, label_mask, is_train=False)
            loss_meter.add(loss.item())
...
if __name__ == "__main__":
    ...
    train_loader, test_loader = get_dataset(args)  # 加载数据 [img, img_mask, label, label_mask]
